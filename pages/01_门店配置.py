...
from db.supabase_client import get_stores, create_store, update_store
...
with st.expander("📈 高峰时段", expanded=False):
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.text_input("午高峰", value="12:00-14:00", disabled=True)
    with col2:
        st.text_input("晚高峰", value="17:00-19:00", disabled=True)
    st.markdown("**周末高峰**")
    col1, col2 = st.columns(2)
    with col1:
        st.text_input("周末午高峰", value="11:00-14:00", disabled=True)
    with col2:
        st.text_input("周末晚高峰", value="16:00-20:00", disabled=True)
